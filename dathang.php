...
if (isset($_SESSION['DonHang'])==false){ 
   header("location:/xemohinh/"); exit();
}
$error = array();
$xe->LuuDonHang($error); //lưu thông tin đơn hàng 
if (count($error)==0){
	$xe->LuuChiTietDonHang(); //lưu các sản phẩm user đã mua
	unset($_SESSION['dayTenSP']);//hủy th.tin đã lưu  trong session
